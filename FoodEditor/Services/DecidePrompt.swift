...
enum DecidePrompt {
    ...
    Return ONLY a valid JSON object — no prose, no markdown. The decisions object:
    {
      "recommended_duration": <number of seconds for the final video>,
      "recommended_hook": "<one line: the opening moment you chose and why>",
      "hook_id": <the shot id that opens the video — must equal final_edit_order[0]>,
      "cold_open": [<shot ids that play first — a PREFIX of final_edit_order; usually 1-3>],
      "final_edit_order": [<the full ordered list of KEPT shot ids, in play order>],
      "trims": [ { "shot_id": <id>, "trim_to_seconds": <absolute proxy second to STOP at>, "reason": "..." } ],
      "voiceovers": [ { "shot_id": <talking-head id to play over b-roll>, "reason": "..." } ],
      "broll_placements": [ { "over_shot_id": <talking-head id>, "broll_shot_id": <visual id to show over it>, "start_offset_seconds": <seconds INTO the over-shot>, "duration_seconds": <1.5-3>, "reason": "..." } ],
      "edit_notes": [ { "shot_id": <id>, "note": "..." } ],
      "style_match_notes": "<which style/brief items you honored and any the footage didn't allow>",
      "video_summary": "<one sentence; copy the index's if unsure>"
    }

    KEEP/CUT IS IMPLICIT: a shot is KEPT if and only if it appears in final_edit_order OR is used as a broll_shot_id. Everything else is dropped. So final_edit_order + broll_placements ARE your keep/cut decision — there is no separate keep field. The sparse arrays (trims/voiceovers/edit_notes) name only the few shots they apply to.

    === HOW TO READ THE CONTENT INDEX ===
    ...
    talk_spans[] = the DEMAND (what's being SAID, sentence by sentence):
    - references_subject / also_references = what the speaker is talking about RIGHT NOW.
    - spoken_text = the words (your source for the hook line); is_to_camera.
    - A talking-head shot's words are the talk_spans whose times overlap that shot's [start, end].

    === BUILD THE NARRATIVE ARC ===
    You are assembling a STORY, not a pile of clips — and VIRALITY NEVER COMES AT THE COST OF COHERENCE: the cut must FLOW and make sense to someone watching start-to-finish (a fragmented highlight-reel kills retention). The shape below is the hard skeleton — build it (adapt to the footage — single dish, multi-dish, recipe, vlog):
    1) HOOK (final_edit_order[0] = hook_id): the first 1.5 SECONDS decide everything. Open on the most ARRESTING moment in the footage — a bold claim ("this might be the best chicken sandwich I've ever had", "this looks insane", "9.5 out of 10"), a big first-taste / peak reaction, or a striking food shot WITH a punchy line over it. NEVER open on a static silent shot, and NEVER on flat audio (a list of dishes, "okay so…", throat-clearing). If the brief REQUIRES an opening shot type (e.g. a food close-up) whose own audio is weak, either TRIM that shot so its audio lands on the strongest nearby line, or pick the shot of that type whose overlapping talk_span is the punchiest claim/reaction.
    2) COLD OPEN (cold_open): hook + a VERDICT TEASE — promise the payoff up front so they keep watching to find out. ACTIVELY HUNT the footage for the single strongest stakes/tease line — a rating hint, "is it worth the hype?", "best I've ever had", "…and it did NOT disappoint" — and seed it into the cold open using the creator's OWN words (never invent text). Hook + one line of context + the tease, then move on. These play FIRST even if a shot's section is "end" — but the tease only HINTS with a stakes/anticipation line ("is it worth the hype?", "set the bar pretty high"). HARD RULE: NEVER place the shot that delivers the actual final rating/score (e.g. "9.5 out of 10") in the cold open — that exact shot is RESERVED for the ending; spend it up front and the video has no payoff left. The single overall verdict/rating is ALWAYS revealed LAST.
    3) PER-DISH WHOLE BLOCKS: after the cold open, group each subject's shots together by topic and keep each block WHOLE — intro → bite → reaction → description — in a sensible dish order. NEVER interleave two dishes, and NEVER strand a dish's reaction after the next dish has already started. NO scattered-highlight montage — every cut must make sense to someone watching straight through; if a moment wouldn't read in sequence, it doesn't belong.
    ...
    - TRIM LONG BITES/REACTIONS: a bite or chew-reaction is best at ~2-4 SECONDS of the PEAK. If a bite-reaction shot runs 7-8 seconds, set trim_to_seconds to clip it to the peak — never ship an 8-second chew.
    - PICK THE BEST, NOT THE FIRST: when a dish is tasted or described more than once, KEEP the moment with the most VIVID reaction or most specific praise (e.g. "so much flavor", "the slaw isn't overpowering", "worth the hype") and drop the generic/duplicate ones. Do NOT drop a whole dish that has a strong reaction just to save time — trim the weaker moments of an OVER-covered dish first.
    ...
    - In style_match_notes, briefly say which items you honored and any the footage didn't allow. If no style profile or brief is present above, use good general short-form food-video judgement.

    The CONTENT INDEX follows. Return ONLY the decisions JSON.
    """

    /// Mirror of `decide-schema.json` in the `[String: Any]` shape `GeminiService` sends as `responseSchema`.
    static var schema: [String: Any] {
        func obj(_ props: [String: Any], _ order: [String]) -> [String: Any] {
            ["type": "OBJECT", "properties": props, "propertyOrdering": order, "required": order]
        }
        let trim = obj(["shot_id": ["type": "INTEGER"], "trim_to_seconds": ["type": "NUMBER"], "reason": ["type": "STRING"]],
                       ["shot_id", "trim_to_seconds", "reason"])
        let voiceover = obj(["shot_id": ["type": "INTEGER"], "reason": ["type": "STRING"]], ["shot_id", "reason"])
        let broll = obj([
            "over_shot_id":         ["type": "INTEGER"],
            "broll_shot_id":        ["type": "INTEGER"],
            "start_offset_seconds": ["type": "NUMBER"],
            "duration_seconds":     ["type": "NUMBER"],
            "reason":               ["type": "STRING"]
        ], ["over_shot_id", "broll_shot_id", "start_offset_seconds", "duration_seconds", "reason"])
        let note = obj(["shot_id": ["type": "INTEGER"], "note": ["type": "STRING"]], ["shot_id", "note"])

        let order = ["recommended_duration", "recommended_hook", "hook_id", "cold_open", "final_edit_order",
                     "trims", "voiceovers", "broll_placements", "edit_notes", "style_match_notes", "video_summary"]
        return obj([
            "recommended_duration": ["type": "NUMBER"],
            "recommended_hook":     ["type": "STRING"],
            "hook_id":              ["type": "INTEGER"],
            "cold_open":            ["type": "ARRAY", "items": ["type": "INTEGER"]],
            "final_edit_order":     ["type": "ARRAY", "items": ["type": "INTEGER"]],
            "trims":                ["type": "ARRAY", "items": trim],
            "voiceovers":           ["type": "ARRAY", "items": voiceover],
            "broll_placements":     ["type": "ARRAY", "items": broll],
            "edit_notes":           ["type": "ARRAY", "items": note],
            "style_match_notes":    ["type": "STRING"],
